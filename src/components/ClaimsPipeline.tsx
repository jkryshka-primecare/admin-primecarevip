    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="glass-panel rounded-lg p-6">
          ...
          <p className="text-3xl font-mono tracking-tighter text-cyan-clinical">{primeCareRate}%</p>
          <div className="mt-3 h-1.5 bg-muted rounded-full overflow-hidden">
            <div className="h-full bg-cyan-clinical rounded-full" style={{ width: `${primeCareRate}%` }} />
          </div>
        </div>
        <div className="glass-panel rounded-lg p-6">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Hero Healthcare Rate</p>
          <p className="text-3xl font-mono tracking-tighter text-sapphire">{heroRate}%</p>
          <div className="mt-3 h-1.5 bg-muted rounded-full overflow-hidden">
            <div className="h-full bg-sapphire rounded-full" style={{ width: `${heroRate}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-lg overflow-hidden">
        <div className="px-6 py-5 border-b border-border flex items-center justify-between">
          ...
          <button className="px-5 py-2.5 bg-foreground text-background text-xs font-bold rounded hover:opacity-90 transition-opacity">
            Export Report
          </button>
        </div>
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-border font-mono text-xs">
            {claims.map((claim) => (
              <tr key={claim.id} className="hover:bg-sapphire/5 transition-colors cursor-pointer">
                ...
                <td className={cn("px-6 py-4 text-right", claim.integrity >= 95 ? "text-emerald-400" : "text-amber-500")}>
                  {claim.integrity}%
                </td>
              </tr>
            ))}
