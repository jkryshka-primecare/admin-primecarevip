...
const CostSavingsPanel = () => {
  ...
  return (
    <div className="space-y-8">
      {/* Grand Total */}
      <div className="glass-panel rounded-lg p-6 border-l-4 border-l-cyan-clinical">
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Total Annualized Savings</p>
        <p className="text-4xl font-mono tracking-tighter text-cyan-clinical">${(grandTotal / 1000000).toFixed(2)}M</p>
          ...
          <div className="space-y-3">
            {savingsData.internalFeatures.map((f) => (
              <div key={f.feature} className="flex justify-between items-center">
                ...
                <span className="text-xs font-mono text-cyan-clinical">{formatCurrency(f.savings)}</span>
              </div>
            ))}
            <div className="border-t border-border pt-3 flex justify-between">
              <span className="text-xs font-bold text-foreground">Total Internal</span>
              <span className="text-sm font-mono font-bold text-cyan-clinical">{formatCurrency(totalInternal)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel rounded-lg p-6">
          ...
          <div className="space-y-3">
            {savingsData.externalFeatures.map((f) => (
              <div key={f.feature} className="flex justify-between items-center">
                <span className="text-xs text-foreground/80">{f.feature}</span>
                <span className="text-xs font-mono text-sapphire">{formatCurrency(f.savings)}</span>
              </div>
            ))}
            <div className="border-t border-border pt-3 flex justify-between">
              <span className="text-xs font-bold text-foreground">Total External</span>
              <span className="text-sm font-mono font-bold text-sapphire">{formatCurrency(totalExternal)}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-8">
          <div>
            ...
            <div className="mt-3 h-2 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-cyan-clinical rounded-full" style={{ width: `${(savingsData.claimsComparison.primeCareVIP.actual / savingsData.claimsComparison.primeCareVIP.benchmark) * 100}%` }} />
            </div>
            <p className="text-[10px] text-cyan-clinical mt-1.5">
          ...
          <div>
            ...
            <div className="mt-3 h-2 bg-muted rounded-full overflow-hidden">
              <div className="h-full bg-sapphire rounded-full" style={{ width: `${(savingsData.claimsComparison.heroHealthcare.actual / savingsData.claimsComparison.heroHealthcare.benchmark) * 100}%` }} />
            </div>
            <p className="text-[10px] text-sapphire mt-1.5">
